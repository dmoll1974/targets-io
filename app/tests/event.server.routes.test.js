'use strict';
var should = require('should'), request = require('supertest'), app = require('../../server'), mongoose = require('mongoose'), User = mongoose.model('User'), Event = mongoose.model('Event'), agent = request.agent(app);
/**
 * Globals
 */
...
describe('Event CRUD tests', function () {
  ...
  it('should be able to save Event instance if logged in', function (done) {
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      var userId = user.id;
      // Save a new Event
      agent.post('/events').send(event).expect(200).end(function (eventSaveErr, eventSaveRes) {
        // Handle Event save error
        if (eventSaveErr)
          done(eventSaveErr);
        // Get a list of Events
        agent.get('/events').end(function (eventsGetErr, eventsGetRes) {
          // Handle Event save error
          if (eventsGetErr)
            done(eventsGetErr);
          // Get Events list
  ...
  it('should not be able to save Event instance if not logged in', function (done) {
    agent.post('/events').send(event).expect(401).end(function (eventSaveErr, eventSaveRes) {
      // Call the assertion callback
      done(eventSaveErr);
    });
  });
  it('should not be able to save Event instance if no name is provided', function (done) {
    ...
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      var userId = user.id;
      // Save a new Event
      agent.post('/events').send(event).expect(400).end(function (eventSaveErr, eventSaveRes) {
  ...
  it('should be able to update Event instance if signed in', function (done) {
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      agent.post('/events').send(event).expect(200).end(function (eventSaveErr, eventSaveRes) {
        ...
        agent.put('/events/' + eventSaveRes.body._id).send(event).expect(200).end(function (eventUpdateErr, eventUpdateRes) {
  ...
  it('should be able to get a list of Events if not signed in', function (done) {
    ...
    eventObj.save(function () {
      // Request Events
      request(app).get('/events').end(function (req, res) {
        // Set assertion
  ...
  it('should be able to get a single Event if not signed in', function (done) {
    ...
    // Save the Event
    eventObj.save(function () {
      request(app).get('/events/' + eventObj._id).end(function (req, res) {
        // Set assertion
  ...
  it('should be able to delete Event instance if signed in', function (done) {
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      agent.post('/events').send(event).expect(200).end(function (eventSaveErr, eventSaveRes) {
        ...
        agent.delete('/events/' + eventSaveRes.body._id).send(event).expect(200).end(function (eventDeleteErr, eventDeleteRes) {
  ...
  it('should not be able to delete Event instance if not signed in', function (done) {
    ...
    eventObj.save(function () {
      // Try deleting Event
      request(app).delete('/events/' + eventObj._id).expect(401).end(function (eventDeleteErr, eventDeleteRes) {
        // Set message assertion
