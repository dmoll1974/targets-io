'use strict';
var should = require('should'), request = require('supertest'), app = require('../../server'), mongoose = require('mongoose'), User = mongoose.model('User'), Dashboard = mongoose.model('Dashboard'), agent = request.agent(app);
/**
 * Globals
 */
...
describe('Dashboard CRUD tests', function () {
  ...
  it('should be able to save Dashboard instance if logged in', function (done) {
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      // Handle signin error
      if (signinErr)
        done(signinErr);
      // Get the userId
      ...
      agent.post('/dashboards').send(dashboard).expect(200).end(function (dashboardSaveErr, dashboardSaveRes) {
        // Handle Dashboard save error
        if (dashboardSaveErr)
          done(dashboardSaveErr);
        // Get a list of Dashboards
        agent.get('/dashboards').end(function (dashboardsGetErr, dashboardsGetRes) {
          // Handle Dashboard save error
          if (dashboardsGetErr)
            done(dashboardsGetErr);
          // Get Dashboards list
  ...
  it('should not be able to save Dashboard instance if not logged in', function (done) {
    agent.post('/dashboards').send(dashboard).expect(401).end(function (dashboardSaveErr, dashboardSaveRes) {
      // Call the assertion callback
      done(dashboardSaveErr);
    });
  });
  it('should not be able to save Dashboard instance if no name is provided', function (done) {
    ...
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      var userId = user.id;
      // Save a new Dashboard
      agent.post('/dashboards').send(dashboard).expect(400).end(function (dashboardSaveErr, dashboardSaveRes) {
  ...
  it('should be able to update Dashboard instance if signed in', function (done) {
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      agent.post('/dashboards').send(dashboard).expect(200).end(function (dashboardSaveErr, dashboardSaveRes) {
        ...
        agent.put('/dashboards/' + dashboardSaveRes.body._id).send(dashboard).expect(200).end(function (dashboardUpdateErr, dashboardUpdateRes) {
          // Handle Dashboard update error
          if (dashboardUpdateErr)
            done(dashboardUpdateErr);
          // Set assertions
  ...
  it('should be able to get a list of Dashboards if not signed in', function (done) {
    ...
    dashboardObj.save(function () {
      // Request Dashboards
      request(app).get('/dashboards').end(function (req, res) {
        // Set assertion
  ...
  it('should be able to get a single Dashboard if not signed in', function (done) {
    ...
    // Save the Dashboard
    dashboardObj.save(function () {
      request(app).get('/dashboards/' + dashboardObj._id).end(function (req, res) {
        // Set assertion
  ...
  it('should be able to delete Dashboard instance if signed in', function (done) {
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      agent.post('/dashboards').send(dashboard).expect(200).end(function (dashboardSaveErr, dashboardSaveRes) {
        ...
        agent.delete('/dashboards/' + dashboardSaveRes.body._id).send(dashboard).expect(200).end(function (dashboardDeleteErr, dashboardDeleteRes) {
  ...
  it('should not be able to delete Dashboard instance if not signed in', function (done) {
    ...
    dashboardObj.save(function () {
      ...
      request(app).delete('/dashboards/' + dashboardObj._id).expect(401).end(function (dashboardDeleteErr, dashboardDeleteRes) {
        // Set message assertion
