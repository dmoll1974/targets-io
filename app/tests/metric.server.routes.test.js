'use strict';
var should = require('should'), request = require('supertest'), app = require('../../server'), mongoose = require('mongoose'), User = mongoose.model('User'), Metric = mongoose.model('Metric'), agent = request.agent(app);
/**
 * Globals
 */
...
describe('Metric CRUD tests', function () {
  ...
  it('should be able to save Metric instance if logged in', function (done) {
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      var userId = user.id;
      // Save a new Metric
      agent.post('/metrics').send(metric).expect(200).end(function (metricSaveErr, metricSaveRes) {
        // Handle Metric save error
        if (metricSaveErr)
          done(metricSaveErr);
        // Get a list of Metrics
        agent.get('/metrics').end(function (metricsGetErr, metricsGetRes) {
          // Handle Metric save error
          if (metricsGetErr)
            done(metricsGetErr);
          // Get Metrics list
  ...
  it('should not be able to save Metric instance if not logged in', function (done) {
    agent.post('/metrics').send(metric).expect(401).end(function (metricSaveErr, metricSaveRes) {
      // Call the assertion callback
      done(metricSaveErr);
    });
  });
  it('should not be able to save Metric instance if no name is provided', function (done) {
    ...
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      var userId = user.id;
      // Save a new Metric
      agent.post('/metrics').send(metric).expect(400).end(function (metricSaveErr, metricSaveRes) {
  ...
  it('should be able to update Metric instance if signed in', function (done) {
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      agent.post('/metrics').send(metric).expect(200).end(function (metricSaveErr, metricSaveRes) {
        ...
        agent.put('/metrics/' + metricSaveRes.body._id).send(metric).expect(200).end(function (metricUpdateErr, metricUpdateRes) {
          // Handle Metric update error
          if (metricUpdateErr)
            done(metricUpdateErr);
          // Set assertions
  ...
  it('should be able to get a list of Metrics if not signed in', function (done) {
    ...
    metricObj.save(function () {
      // Request Metrics
      request(app).get('/metrics').end(function (req, res) {
        // Set assertion
  ...
  it('should be able to get a single Metric if not signed in', function (done) {
    ...
    // Save the Metric
    metricObj.save(function () {
      request(app).get('/metrics/' + metricObj._id).end(function (req, res) {
        // Set assertion
  ...
  it('should be able to delete Metric instance if signed in', function (done) {
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      agent.post('/metrics').send(metric).expect(200).end(function (metricSaveErr, metricSaveRes) {
        ...
        agent.delete('/metrics/' + metricSaveRes.body._id).send(metric).expect(200).end(function (metricDeleteErr, metricDeleteRes) {
  ...
  it('should not be able to delete Metric instance if not signed in', function (done) {
    ...
    metricObj.save(function () {
      // Try deleting Metric
      request(app).delete('/metrics/' + metricObj._id).expect(401).end(function (metricDeleteErr, metricDeleteRes) {
        // Set message assertion
