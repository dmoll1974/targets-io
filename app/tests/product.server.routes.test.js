'use strict';
var should = require('should'), request = require('supertest'), app = require('../../server'), mongoose = require('mongoose'), User = mongoose.model('User'), Product = mongoose.model('Product'), agent = request.agent(app);
/**
 * Globals
 */
...
describe('Product CRUD tests', function () {
  ...
  it('should be able to save Product instance if logged in', function (done) {
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      var userId = user.id;
      // Save a new Product
      agent.post('/products').send(product).expect(200).end(function (productSaveErr, productSaveRes) {
        // Handle Product save error
        if (productSaveErr)
          done(productSaveErr);
        // Get a list of Products
        agent.get('/products').end(function (productsGetErr, productsGetRes) {
          // Handle Product save error
          if (productsGetErr)
            done(productsGetErr);
          // Get Products list
  ...
  it('should not be able to save Product instance if not logged in', function (done) {
    agent.post('/products').send(product).expect(401).end(function (productSaveErr, productSaveRes) {
      // Call the assertion callback
      done(productSaveErr);
    });
  });
  it('should not be able to save Product instance if no name is provided', function (done) {
    ...
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      var userId = user.id;
      // Save a new Product
      agent.post('/products').send(product).expect(400).end(function (productSaveErr, productSaveRes) {
  ...
  it('should be able to update Product instance if signed in', function (done) {
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      agent.post('/products').send(product).expect(200).end(function (productSaveErr, productSaveRes) {
        ...
        agent.put('/products/' + productSaveRes.body._id).send(product).expect(200).end(function (productUpdateErr, productUpdateRes) {
          // Handle Product update error
          if (productUpdateErr)
            done(productUpdateErr);
          // Set assertions
  ...
  it('should be able to get a list of Products if not signed in', function (done) {
    ...
    productObj.save(function () {
      // Request Products
      request(app).get('/products').end(function (req, res) {
        // Set assertion
  ...
  it('should be able to get a single Product if not signed in', function (done) {
    ...
    // Save the Product
    productObj.save(function () {
      request(app).get('/products/' + productObj._id).end(function (req, res) {
        // Set assertion
  ...
  it('should be able to delete Product instance if signed in', function (done) {
    agent.post('/auth/signin').send(credentials).expect(200).end(function (signinErr, signinRes) {
      ...
      agent.post('/products').send(product).expect(200).end(function (productSaveErr, productSaveRes) {
        ...
        agent.delete('/products/' + productSaveRes.body._id).send(product).expect(200).end(function (productDeleteErr, productDeleteRes) {
  ...
  it('should not be able to delete Product instance if not signed in', function (done) {
    ...
    productObj.save(function () {
      ...
      request(app).delete('/products/' + productObj._id).expect(401).end(function (productDeleteErr, productDeleteRes) {
        // Set message assertion
