                      ...
                      if (serie.name === existingSerie.name) {
                        var newDatapoints = _.filter(serie.data, function (newDataPoint) {
                          ...
                          _.each(existingSerie.data, function (existingDataPoint) {
                            if (newDataPoint[0] === existingDataPoint.x)
                              isNew = false;
                          });
                          return isNew;
                        });
                        if (newDatapoints.length > 0) {
                          _.each(newDatapoints, function (datapoint) {
                            chart.series[i].addPoint([
                              datapoint[0],
                              datapoint[1]
                            ], true, true, true);  //chart.series[i].data.push([datapoint[0], datapoint[1]]);
                          });
                        }  //return;
                      }
                    });
        ...
        events: {
          setExtremes: function (e) {
            ...
            if ($scope.zoomLock === true) {
              ...
              var chart = angular.element($scope.graphSelector).highcharts('StockChart', $scope.config).css('width', '100%');
              while (chart.series.length > 0) {
                chart.series[0].remove(false);  //deletes all series
              }
              chart.showLoading('Loading data ...');
